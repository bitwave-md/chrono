...
const sections = [
  ...
  { title: "Workspace", operatorOnly: false, workspaceOnly: true, items: [
  ...
  { title: "Administration", operatorOnly: true, workspaceOnly: true, items: [
...
export function SettingsSidebar({ workspace }: { workspace: WorkspaceIdentity }) {
  ...
  const visibleSections = sections.filter((section) =>
    (!section.operatorOnly || workspace.isOperator)
    && (!section.workspaceOnly || workspace.role !== "guest"),
  );
  ...
  return (
    <Sidebar className="border-r border-sidebar-border" collapsible="offcanvas">
      ...
      <SidebarContent>
        ...
        {normalized && !visibleSections.some((section) => section.items.some((item) => item.label.toLowerCase().includes(normalized))) ? <p className="px-4 py-8 text-center text-xs text-muted-foreground">No settings found.</p> : null}
      </SidebarContent>
    </Sidebar>
  );
}
