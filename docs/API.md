...
- `GET /api/workspaces/:workspaceSlug/settings/storage` and `/updates` expose
  ...
  heartbeat-backed updater capability, and persisted job progress. A writable
  queue without a live Docker-capable worker is reported as manual recovery,
  never automatic readiness. Terminal failures include a
